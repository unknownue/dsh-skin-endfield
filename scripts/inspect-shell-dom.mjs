/**
 * Inspector for the shell DOM: dumps the hooks and computed values the decor layer
 * has to address, so a rule can be aimed at a real anchor instead of a guess.
 *
 * Not a check -- it asserts nothing. It is the tool that produced the facts behind
 * several sidebar decisions: the data-slot names, the projectRow/sessionRow split, the
 * 12px/8px radii that had to be squared, and which single row still painted an edge.
 *
 * Run: $env:DSH_URL='...token=...'; node scripts/inspect-shell-dom.mjs
 */
