...
`pnpm verify`（`scripts/verify-all.ps1`）按代价从低到高跑，**24 步**（离线 7 + live 17）：
...
| live | `verify-title-underline-live.mjs` **12** | 标题下的**当前位置实线**：2px 实心、用皮肤焦点色（**不是**用户配的 accent）、是 overlay（不会推动标题）、**离顶栏分隔线留足 12px**、两端内缩（所以量的是文字盒而不是整块 chip）、**跟着标题宽度变化**（短 < 当前 < 长，长标题截断时钳在文字盒） |
...
| 颜色用 `--endfield-focus` | 皮肤给"选中/焦点族"留的颜色，和第 15 节顶部那条注释一致（`///`、这条线、基线分隔线都归它）；**不用**品牌黄、也**不用**用户配置的 accent —— 后者一变会把这条标记一起重绘 |

`verify-title-underline-live.mjs`（12 项）把它钉住，其中三条是"看着对、量出来才知道"的：**离分隔线的余量**（< 8px 就报错）、**跟着标题宽度变**（短标题必须给出更窄的线）、以及**线宽恰好等于文字盒**。这条规则如果哪天跑到分隔线上，就会从"标签下的标记"变成"顶栏的第二条边框" —— 这正是那几条断言存在的意义。
