...
| live | `verify-header-tabs-live.mjs` **10** | 顶栏单位行（Chat / Trajectory / Files / Tasks / Papers）：**非激活单元会回应指针**（中性洗色 + 字色提亮，实测 `rgba(217,217,217,0.22)` → 合成后比 band 亮 **38 级平均通道**，字色 `#999999 → #F2F2F2`），而**当前单元保持自己的 plate 不被染色**；整行居中于顶栏、每个单元直角、当前 plate 的墨色与底色不同且**对比度 10:1** |
...
这一行原来有一个**交互缺口**：皮肤在改造顶栏时把外壳自己的 hover 底色连同其它 plate 一起移除了，却**没有补上替代**，于是五个单元看起来像五个标签、不像五个控件。现在补的是一条**皮肤自己的**中性洗色 `rgba(217,217,217,0.22)` ＋字色提亮到主字色；**当前单元排除在外** —— 它本身就是一块彩色 plate，再叠洗色只会把自己压暗。

> **洗色为什么不用外壳那个 token**：`--dsw-alias-interactive-bg-hover` 是 `rgba(217,217,217,0.08)`，合成到 band 上只有 `#2C2C2C` vs `#222222` —— **约 10 级平均通道**，在深色画布上基本看不见（这正是"背景太浅"的反馈）。现在用 0.22，合成后是 `#393939`，比 band 亮 **38 级**，既能看见、又不至于和当前单元的 accent plate 抢。检查里把这条写成了**可见度断言**（≥ 20 级，按合成值算，因为半透明洗色不能和实色直接比）—— 原来的 10 级过不去、现在的 38 级过得去。
