...
function registerSkinSettings(ctx) {
	if (ctx.settings === void 0) {
		ctx.logger?.warn?.("dsh-skin-endfield: no settings service composed — the Skin settings page will be inert and the skin uses its built-in defaults.");
		return false;
	}
	try {
		...
		ctx.logger?.warn?.(`dsh-skin-endfield: settings registration failed (${error instanceof Error ? error.message : String(error)}) — the skin continues on its built-in defaults.`);
		return false;
	}
}
function apply(ctx) {
